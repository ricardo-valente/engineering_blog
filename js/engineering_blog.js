...
function headerNav() {
   var $main_nav = $body.find('.main-nav');
   expandSection('.icon-menu', '.main-nav, .icon-logo', 'js-expanded');
   $body.find('.icon-menu').on('click', function() {
      if( $main_nav.hasClass('js-expanded') ) {
         $body.find('.icon-menu, .icon-logo').delay(1000).queue(function (next) {
            $body.find('.icon-menu, .icon-logo').css('z-index', '1');
            next();
         });
         // $body.find('.icon-menu, .icon-logo').css('z-index', '1');
      } else {
         $body.find('.icon-menu, .icon-logo').css('z-index', '1000');
      }
   });
}
